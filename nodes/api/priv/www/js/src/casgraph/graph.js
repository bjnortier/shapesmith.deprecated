define(['underscore', 'backbone-events', 'src/casgraph/sha1hasher'], 
    ...
    var CAS = function(options) {
        ...
        this.put = function(object) {
            if (!object.hasOwnProperty(idKey)) {
                throw Error("object has no '" + idKey + "' property");
            }
            var id = object[idKey];

            if (vertices[id] !== undefined) {
                 throw Error("object with id '" + id + "' already in graph");
            }
        ...
        this.remove = function(object) {
            ...
            if (vertices[removeId] === undefined) {
                 throw Error("no object '" + removeId + "' in graph");
            }
        ...
        this.replace = function(object) {
            ...
            if (vertices[id] === undefined) {
                 throw Error("no object '" + id + "' in graph");
            }
            var original = vertices[id];
            vertices[id] = object;
            this.hashIfHashable(id, object);
            updateLeafFirstSequence();
        }
        ...
        this.createEdge = function(from, to) {
            ...
            if (!outgoingVertices[fromId]) {
               throw Error("no object '" + fromId + "' in graph");
            }
            if (!incomingVertices[toId]) {
               throw Error("no object '" + toId + "' in graph");
            }
        ...
        this.removeEdge = function(from, to) {
            ...
            if (index === -1) {
                throw Error('no edge from ' + fromId + ' to ' + toId);
            }
        ...
        this.getOutgoing = function(from) {
            var fromId = from[idKey];
            if (!outgoingVertices[fromId]) {
               throw Error('no vertex ' + fromId + ' in graph');
            }
        ...
        this.fromHashSerialization = function(hashedGraph, hashesToVertices) {
            if (_.keys(vertices).length) {
                throw Error('Cannot reconstruct - graph not empty');
            }
        ...
        this.diffFrom = function(fromGraph, listener) {
            ...
            if (hashFn(fromMetadata) !== hashFn(toMetadata)) {
                listener({metadataChanged: {from: fromMetadata, to: toMetadata}});
            }
