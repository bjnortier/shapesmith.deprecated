    ...
    var GeometryGraph = function() {
        ...
        var graph = new CASGraph({
            ...
            constructionFn: function(obj) {
                if (!geomNode.constructors[obj.type]) {
                    throw Error('no GeomNode constructor for ' + obj.type);
                }
        ...
        this.undo = function() {
            if (currentSnapshotIndex === 0) {
                throw Error('no snapshot to undo to');
            }
            var from = snapshots[currentSnapshotIndex];

            this.replicator && this.replicator.detachFrom(graph);
            graph = snapshots[--currentSnapshotIndex].clone();
            varGraph = new variableGraphLib.Graph(graph);
            this.replicator && this.replicator.attachTo(graph);
        ...
        this.redo = function() {
            if (currentSnapshotIndex === (snapshots.length-1)) {
                throw Error('no snapshot to redo to');
            }
            var from = snapshots[currentSnapshotIndex];
