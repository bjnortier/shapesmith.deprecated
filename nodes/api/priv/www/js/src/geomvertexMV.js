define([
        ...
        'src/asyncAPI',
        'src/layers/layertreedragdropmixin',
    ...
    var Common = {
        ...
        replaceWithDisplay: function(original, replacement) {
            var modelToDestroy = VertexMV.getModelForVertex(original);
            var rowIndex, containerElement;
            if (modelToDestroy.domView) {
                rowIndex = modelToDestroy.domView.$el.index();
                containerElement = modelToDestroy.domView.$el.closest('.layer-container');
            }
        ...
        replaceOrAppendInTable: function(view, defaultContainerSelector) {
            var rowIndex = view.model.attributes.rowIndex;
            var containerElement = view.model.attributes.containerElement;
            if (containerElement !== undefined) {
                if ((rowIndex !== undefined) && (rowIndex < containerElement.children().length)) {
                    $(containerElement.children()[rowIndex]).before(view.$el);
                } else {
                    containerElement.append(view.$el);
                }
            } else {
                $(defaultContainerSelector).append(view.$el);
            }
        },
    ...
    var EditingDOMView = VertexMV.EditingDOMView.extend({
        ...
        initialize: function() {
            ...
            if (this.model.parentModel) {
                ...
                this.model.replaceOrAppendInTable(this, this.defaultDOMContainer || '#geometry'); 
            }
        },
    ...
    var DisplayModel = VertexMV.DisplayModel.extend({ 
        ...
        select: function(ids, selection) {
            ...
            if ((selection.length === 1) && this.selected) {
                this.replaceWithEditing(this.vertex, AsyncAPI.edit(this.vertex));
            }
        },
    ...
    var DisplayDOMView = VertexMV.DisplayDOMView.extend({
        ...
        dragstart: function(e) {
            e.stopPropagation();
            VertexMV.eventProxy.trigger('geometryDragStarted', this);
            dragSrcView = this;
        },
        ...
        dragover: function(e) {
            ...
            this.$el.removeClass('validSiblingBeforeDropTarget');
            this.$el.removeClass('validSiblingAfterDropTarget');
            if (e.originalEvent.offsetY < 5) {
                if (this.isValidSiblingDrop(dragSrcView, this)) {
                    this.$el.removeClass('validSiblingAfterDropTarget')
                    this.$el.addClass('validSiblingBeforeDropTarget');
                }
            } else if (e.originalEvent.offsetY > height - 5) {
                if (this.isValidSiblingDrop(dragSrcView, this)) {
        ...
        drop: function(e) {
            ...
            // Drag container into geometry
            if (!dragSrcView) {
                return;
            }

            var source = this.model.createOrGetGeomNode(dragSrcView.model.vertex.id);
            var target = this.model.createOrGetGeomNode(this.model.vertex.id);
            if (this.$el.hasClass('validSiblingBeforeDropTarget')) {
                this.model.moveFirstBeforeSecond(source, target);
            } else if (this.$el.hasClass('validSiblingAfterDropTarget')) {
                this.model.moveFirstAfterSecond(source, target);
            }
