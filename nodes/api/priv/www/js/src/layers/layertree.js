define(['underscore', 'backbone-events'], function(_, Events) {
    ...
    var Container = function(type, namerOrName) {
        ...
        this.insertBefore = function(node, target) {
            ...
            if (index === -1) {
                throw Error('node not found: ' + (target.name ? target.name : target.id));
            }
        ...
        this.detach = function(node) {
            ...
            if (index === -1) {
                throw Error('node not found: ' + (node.name ? node.name : node.id));
            }
        ...
        this.remove = function(node) {
            ...
            if (node.getChildren && _.any(node.getChildren(), function(child) {
    ...
    var RootContainer = function() {
        Container.prototype.constructor.call(this, 'root', function() { return 'root'});
    ...
    var Tree = function() {
        ...
        this.moveFirstBeforeSecond = function(node, target) {
            var removed = root.findAndDetach(node);
