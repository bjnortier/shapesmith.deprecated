...
provide_content(ReqData, Context) ->
    ...
    Rendered = api_walrus:render_template(api_views_signup, WalrusContext),
...
accept_content(ReqData, Context) ->
    ...
    case api_db:create_user(Username, Password, EmailAddress) of
	ok ->
	    ReqData1 = api_resource:create_session(ReqData, Username),
	    {true, api_resource:json_response(<<"created">>, ReqData1), Context};
	already_exists ->
	    {{halt, 409}, api_resource:json_response({[{<<"username">>, <<"sorry - this username is already used">>}]}, ReqData), Context}
    end.
